...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedP == null) {
				Position pawnPos;
				...
				capturedP = board.removePiece(pawnPos);
				capturedPieces.add(capturedP);
				piecesOnBoard.remove(capturedP);
			}
		}
	...
	private ChessPiece king(Color color) {
		List<Piece> opponentPieces = piecesOnBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
